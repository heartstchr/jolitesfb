...
<div class="timeline-cover">
    <div class="cover image ">
        ...
        <ul class="list-unstyled">
            <li class="active"><a href="{{URL::Route('profile',array('profile_id' => $user1->id))}}"><i class="fa fa-fw fa-clock-o"></i> <span>Timeline</span></a></li>
...
<!--                <p class="lead margin-none "> <i class="fa fa-quote-left text-muted fa-fw"></i> What a fun Partyyy</p>-->
                ...
                @if(isset($friend->status))
                    @if($friend->user_one_id == $user1->id)
                        @if($friend->status == 1)
                            <div class="btn-group">
                                <a type="button" class="btn btn-primary" href="{{URL::route('confirmFriendRequest',['id' => $user1->id])}}"><i class="fa fa-check"></i> Confirm</a>
                                <a type="button" class="btn btn-default" href="{{URL::Route('cancelFriendRequest',['id' => $user1->id])}}"><i class="fa fa-times"></i> Cancel</a>
                            </div>
                        @elseif($friend->status == 2)
                            <div class="btn-group">
                                <button type="button" class="btn btn-primary"><i class="fa fa-check"></i> Friends</button>
                                <a type="button" class="btn btn-default" href="{{URL::Route('unFriend',['id' => $user1->id])}}">Unfriend</a>
                            </div>
                        @elseif($friend->status == NULL || $friend->status == 3)
                            <a href="{{URL::Route('friendRequest',['id' => $user1->id])}}" class="btn btn-primary">Send Friend Request</a>
                        @endif
                    @else
                        @if($friend->status == 2)
                        <div class="btn-group">
                        ...
                        <div class="btn-group">
                            <button type="button" class="btn btn-primary"><i class="fa fa-check"></i> Friend Request Sent</button>
                            <a type="button" class="btn btn-default" href="{{URL::Route('cancelFriendRequest',['id' => $user1->id])}}"><i class="fa fa-times"></i> Cancel</a>
                        </div>
                        @elseif($friend->status == NULL || $friend->status == 3)
                        <a href="{{URL::Route('friendRequest',['id' => $user1->id])}}" class="btn btn-primary">Send Friend Request</a>
                ...
                @if(Auth::getUser()->role_id==1)
                    <a class="btn btn-danger" href="{{URL::Route('adminDeleteUser',['id'=>$user1->id])}}"> Delete this user </a>
                @endif
                              ...
                              <div class="modal-dialog">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    ...
                                    <h4 class="modal-title" id="myModalLabel"> Send Message to {{ucwords($user1->first_name)}} {{ucwords($user1->last_name)}} </h4>
                                  </div>
                                  <form action="{{URL::Route('sendMessage')}}" method="POST">
                                  <div class="modal-body">

                                        <input type="text" name="receiver" value="{{$user1->id}}" style="display:none;"/>
    ...
    <div class="timeline_sidebar col-xs-12 col-sm-4">
        <div class="widget">
            ...
            <div class="widget-body padding-none">
                @foreach($user1_meta as $user_meta)
